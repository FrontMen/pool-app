...
const Match = ({ match, players }) => {
  const badgeClass=`badge badge-${won(match) ? 'success': 'secondary'} badge-pill`
  return (
     <li class="list-group-item d-flex justify-content-between align-items-center" key={match.matchId}>
      {won(match) ? 'Gewonnen' : 'Verloren'} van {getPlayer(match.opponent, players).name}
